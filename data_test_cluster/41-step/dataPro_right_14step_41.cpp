...
// multi thread
void hash_collision_detection(int thread_id, int num_threads, int64_t dataTotal, int s[], atomic<int>& number_36, atomic<int>& number_37, atomic<int>& number_38, atomic<int>& number_39, atomic<int>& number_40, vector<uint32_t> m0, vector<uint32_t> m1, vector<uint32_t> result) {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<uint32_t> dis(0, UINT32_MAX);

    // the conditions on (Y22, Y23, Y24, Y25, Y26)
    // X[25][20] = X[24][20]
    // X[25][19] = X[24][19]
    string patterns[] = {
        "1==============================1", // Y22
...
int main() {
    ...
    // the modular difference of (Y36, Y37, Y38, Y39, Y40)
    vector<uint32_t> result = {0x0, 0xffffffc0, 0x800001, 0xfffe0000, 0xf0000000};
    // the modular difference of message words
    vector<uint32_t> modularM = {0x8000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    int s[cons] = {7, 6, 15, 13, 11, 9, 7, 15, 11, 8, 6, 6, 14, 12};
    ...
    int test_number = 200;
    vector<double> proList_36(test_number, 0);
    ...
    for (int cishu = 0; cishu < test_number; cishu++){
        ...
        for (int i = 0; i < cons; i++) {
            ...
            // same value for the same message words
            m0[7] = m0[3];
            m1[7] = m1[3];
            m0[12] = m0[2];
            m1[12] = m1[2];
        ...
        for (int i = 0; i < num_threads; i++) {
            threads.push_back(thread(hash_collision_detection, i, num_threads, dataTotal, s, ref(number_36), ref(number_37), ref(number_38),ref(number_39),ref(number_40), m0, m1, result));
        }
        ...
        proList_36[cishu] = static_cast<double>(number_36) / dataTotal;
    ...
    double mean = calculate_mean(proList_36);
    double variance = calculate_variance(proList_36);
    std::cout << "Mean36: " << mean << std::endl;
    std::cout << "Variance36: " << variance << std::endl;

    mean = calculate_mean(proList_37);
    variance = calculate_variance(proList_37);
