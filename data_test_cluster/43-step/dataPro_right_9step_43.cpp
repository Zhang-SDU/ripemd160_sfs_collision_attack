...
void hash_collision_detection(int thread_id, int num_threads, int64_t dataTotal, int s[], uint32_t K, atomic<int>& number_38, atomic<int>& number_39, atomic<int>& number_40,atomic<int>& number_41, atomic<int>& number_42, vector<uint32_t> m0, vector<uint32_t> m1, vector<uint32_t> result) {
...
int main() {
    ...
    // the modular difference of (Y38, Y39, Y40, Y41, Y42)
    vector<uint32_t> result = {0x800001, 0xfffe0000, 0xf0000000, 0xff7ec000, 0xfff00403};
    vector<uint32_t> modularM = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8000};
    int s[cons] = {15, 11, 8, 6, 6, 14, 12, 13, 5};
    ...
    for (int cishu = 0; cishu < test_number; cishu++){
        ...
        for (int i = 0; i < num_threads; i++) {
            threads.push_back(thread(hash_collision_detection, i, num_threads, dataTotal, s, K, ref(number_38),ref(number_39),ref(number_40), ref(number_41), ref(number_42), m0, m1, result));
        }
    ...
    double mean = calculate_mean(proList_38);
    double variance = calculate_variance(proList_38);
