...
class FunctionModel:
    def __init__(self, steps, bounds):
        ...
        self.__boolFunction_left = ["XOR", "IFX", "ONZ", "IFZ", "ONX"]
    ...
    def Object(self, object_value):
        ...
        for i in range(self.__step, self.__bounds_rounds):
            if self.__boolFunction_left[(i - 5) // 16] == "XOR":
                for j in range(32):
    ...
    def main(self):
        ...
        for step in range(0, self.__bounds_rounds):
            ...
            for i in range(32):
                ...
                temp_m.append("m_" + str(self.__OrderMessageWords_left[step]) + "_" + str(i))
            in_var_v_m.append(temp_v_m)
            in_var_d_m.append(temp_d_m)
            in_var_m.append(temp_m)

        for i in range(self.__step, self.__bounds_rounds):
            self.R(self.__boolFunction_left[(i - 5) // 16], self.__isc[i], self.__isf[i], self.__isv[i],
                   self.__isk[i],
        ...
        # value transition model
        # for i in range(self.__step, self.__bounds_rounds):
        #     self.ripemd_value(self.__filterBool_right[(i - 5) // 16], self.__RotateCons_left[i],
        #            in_var_m[i],
        #            self.left_shift(in_var_a[i - 5], 10),
        #            self.left_shift(in_var_a[i - 4], 10),
        #            self.left_shift(in_var_a[i - 3], 10),
        #            in_var_a[i - 2],
        #            in_var_a[i - 1],
    ...
    def solver(self):
        ...
        while True:
            ...
            file_write = open("left_model_43.cvc", "w")
            ...
            file_write.close()
            stp_parameters = ["stp", "left_model_43.cvc", "--cryptominisat", "--threads", "64"]
            R = subprocess.check_output(stp_parameters)

            if "Valid.\n" != R.decode():
                old_name = "left_res2_m12_" + str(pro - 1) + ".out"
                file = open("left_res2_m12_" + str(pro - 1) + ".out", "w")
                file.write(R.decode())
                file.close()
                data_list = open("left_res2_m12_" + str(pro - 1) + ".out", "r").read()
                data = data_list.replace("ASSERT( ", "").replace(" );", "").replace("\nInvalid.", "").split("\n")
                for i in data:
                    if "pro" in i:
                        temp = i.split(" = ")
                ...
                os.rename(old_name, "left_res2_m12_" + str(pro) + ".out")

            else:
                print("the probability of differential characteristic: %s unsatisfied" % (pro - 1))
                break

        for temp in read_differential_characteristic("left_res2_m12_" + str(pro) + ".out", self.__bounds_rounds):
            print(temp)
